...
export async function POST(request: Request) {
  try {
    ...
    const isMedia = file.type.startsWith('audio/') || file.type.startsWith('video/') || /\.(mp3|wav|m4a|aac|ogg|webm|mp4|mov|mkv)$/i.test(file.name)
    if (!isMedia) return NextResponse.json({ error: 'Upload an audio or video file' }, { status: 415 })
